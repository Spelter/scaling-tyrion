...
In the Zugmonitor (see \Ref{fig:zugmonitor}) each long-distance 
...
Kingdom (see \Ref{fig:ukLiveMap}). The plot fetches the departure time from the 
...
This a train graph based on the N-Judah line on the Muni Metro light railway line in San Francisco (see \Ref{fig:muniLightRail}). 
...
to spot the delay of each train. As with \Ref{sub:subsection_ukLiveMap}
...
Because of this, they also collect in data from points that each train passes, as described in \Ref{sec:intro_data_sets}. 
Based on this, they have recently released a map (see \Ref{fig:jernbaneverket-punklighet}) over the punctuality on each trains segment. This is a 
...
trains. The service gathers and processes data from Trafikverket\cite{trafikverket}. As with Cargonet (\Ref{sub:subsection_cargonet}), it 
provides a method (\Ref{fig:taag-info-kart}) of tracking live trains and 
...
and the train operators Cargonet (\Ref{sub:subsection_cargonet}), NSB\cite{nsbForside}. The aim is to systematically improve the precision 
...
points along the route (see \Ref{fig:jernbaneverket-trafikkdata}), 
...
developed a prototype for such a plot, see \Ref{fig:live-punklighet}. Here the 
...
it possible to get a time used over distance plot (see \Ref{fig:plot-spc-for-strekning}) based on the selected data in the Punctuality for routes plot. 
...
As can be seen in \Ref{fig:cargonet}, it only shows a picture of the current
